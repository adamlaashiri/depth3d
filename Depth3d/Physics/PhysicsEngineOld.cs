...
{
    public class PhysicsEngineOld : IDisposable
    {
        ...
        public PhysicsEngineOld(double updateFrequency)
        {
            // create new bufferpool, and thread dispatcher (based on processor count)
            ...
            _timestepDuration = 1f / ((float) updateFrequency);

            // This is where all the simulation is happening
            _simulation = new Simulation();
        }

        public void Init()
        {
            _simulation = Simulation.Create(_bufferPool, new NarrowPhaseCallbacks(), new PoseIntegratorCallbacks(new Vector3(0, -9.82f, 0)), new SolveDescription(8, 1));
        }

        // Body refers to a basic collidable mobile object that can be connected to constraints.
        public BodyHandle AddBody(BodyDescription body)
        {
            return _simulation.Bodies.Add(body);
        }

        public BodyReference GetBodyData(BodyHandle bodyHandle)
        {
            return _simulation.Bodies[bodyHandle];
        }

        public TypedIndex AddShape(Box shape)
        {
            return _simulation.Shapes.Add(shape);
        }

        public void AddStatic(OpenTK.Mathematics.Vector3 position, OpenTK.Mathematics.Vector3 boxShape)
        {
            _simulation.Statics.Add(new StaticDescription(new Vector3(position.X, position.Y, position.Z), _simulation.Shapes.Add(new Box(boxShape.X, boxShape.Y, boxShape.Z))));
        }

        public void Update()
        {
            _simulation.Timestep(_timestepDuration, _threadDispatcher);
        }

        public void UpdateEntities(List<Entities.Entity> entities)
        {
            /*
            foreach (var entity in entities)
            {
                if (entity.Static)
                    continue;
                RigidPose pose = _simulation.Bodies[entity.BodyHandle].Pose;
                OpenTK.Mathematics.Vector3 rotation = Maths.Math.QuaternionToEuler(pose.Orientation.X, pose.Orientation.Y, pose.Orientation.Z, pose.Orientation.W);
                entity.Position = new OpenTK.Mathematics.Vector3(pose.Position.X, pose.Position.Y, pose.Position.Z);
                entity.Rotation = rotation;
            }
            */
        }

        public void AddForce(BodyHandle bodyHandle, Vector3 point)
        {
            _simulation.Bodies[bodyHandle].ApplyAngularImpulse(point);
        }
        ...
        public void CleanUp()
        {
            ...
            Console.WriteLine("Cleaned up physics engine!");
        }
